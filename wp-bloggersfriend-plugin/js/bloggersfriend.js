	...
	$.extend({
		bloggersfriend: {
			...
			initTemplate: function() {
				...
				var linkOpenTag = '<a href="${DetailPageURL}" class="bloggersfriend_link" data-role="bloggersfriend_product" data-asin="${ASIN}" title="${Title}" target="_blank">';
				var smallImageTemplate = [
					'{{if SmallImage}}',
					'{{if $item.setInfoMargin(SmallImage.width+10)}}{{/if}}',
					'<div class="bloggersfriend_image">',
					linkOpenTag,
				...
				var priceContentTemplate = [
					'{{if $item.isSale()}}',
					'<b>' + r.ListPrice + '</b><span class="bloggersfriend_listprice">${ListPrice.FormattedPrice}</span><br/>',
				...
				var priceTemplate = '{{if ListPrice}}<div class="bloggersfriend_price" title="' + r.PriceUsage + '">' + priceContentTemplate + '</div>{{/if}}';
				var priceLiTemplate = '{{if ListPrice}}<li class="bloggersfriend_price" title="' + r.PriceUsage + '">' + priceContentTemplate + '</li>{{/if}}';
				var reviewLinkTemplate = '';
				if (this.isCustomerReviewEnabled) {
					reviewLinkTemplate = '<a href="${IFrameReviewURL}&TB_iframe=true&height=500&width=600" title="' + r.CustomerReviewTitle + '" target="_blank" class="bloggersfriend_review">' + r.SeeCustomerReviews + '</a>';
				}
				...
				var defaultTemplates = {
					Small: [
						'<div class="bloggersfriend_item">',
						imageTemplate,
						'{{if _ShowDefaultImage}}', smallImageTemplate, '{{/if}}',
						'<div class="bloggersfriend_info" style="{{if _InfoMarginLeft}}margin-left:${_InfoMarginLeft}px;{{/if}}">',
						'<h4>', linkTemplate, '</h4>',
						'<ul>',
					...
					Music: [
						'<div class="bloggersfriend_item bloggersfriend_music">',
						imageTemplate,
					...
					DVD: [
						'<div class="bloggersfriend_item bloggersfriend_dvd">',
						imageTemplate,
						'{{if _ShowDefaultImage}}', mediumImageTemplate, '{{/if}}',
						'<div class="bloggersfriend_info" style="{{if _InfoMarginLeft}}margin-left:${_InfoMarginLeft}px;{{/if}}">',
						'<h4>', linkTemplate, '</h4>',
					...
					Book: [
						'<div class="bloggersfriend_item bloggersfriend_book">',
						imageTemplate,
						'{{if _ShowDefaultImage}}', mediumImageTemplate, '{{/if}}',
						'<div class="bloggersfriend_info" style="{{if _InfoMarginLeft}}margin-left:${_InfoMarginLeft}px;{{/if}}">',
						'<h4>', linkTemplate, '</h4>',
			...
			template: function(templates) { // set up jQuery template
				$.each(templates, function(name, tmpl) {
					if (tmpl) {
						$.template('bloggersfriend' + name + 'Tpl', (typeof tmpl === 'string') ? tmpl : tmpl.join(''));
					}
				});
			},
			render: function(items) {
				var $items = [];
				this.initTemplate();
				$(".bloggersfriend_item").each(function() {
					...
					if (item) {
						...
						var $item = $.bloggersfriend.tmpl(item, $.bloggersfriend.formatTmplName(tmpl));
						$(this).replaceWith($item.hide());
						var $review = $item.find('.bloggersfriend_review');
						if (isIE6) {
						...
						if ($.bloggersfriend.isTrackEventEnabled) {
							$item.find('.bloggersfriend_link').click(function() {
								try {
									var data = $(this).data();
									var title = $(this).attr('title');
									if (data.role == 'bloggersfriend_product') {
										if ($.isFunction(ga)) {
											ga('send', 'event', 'bloggersfriend', 'Click', data.asin + ' ' + title);
										} else if (_gaq) {
											_gaq.push(['_trackEvent', 'bloggersfriend', 'Click', data.asin + ' ' + title]);
										}
									}
								} catch (e) {}
							});
				...
				if ($.bloggersfriend.isFadeInEnabled) {
					function fadeIn() {
						if ($items.length > 0) {
			...
			formatTmplName: function(key) {
				return (key) ? 'bloggersfriend' + key + 'Tpl' : null;
			},
			...
			tmpl: function(item, tmpl) {
				...
				return $.tmpl($.template[tmpl] || this.getTemplate(item), item, {
					isSale: function() {
						if (this.data) {
							var lp = this.data.ListPrice,
								la = Number(lp.Amount),
								os = this.data.OfferSummary || {},
								sp = os.LowestNewPrice || {},
								sa = Number(sp.Amount);
			...
			execute: function() {
				var bloggersfriendVars = window.bloggersfriendVars;
				if (bloggersfriendVars) {
					function render() {
						if (!bloggersfriendVars.items) {
							return;
						}
						if ($.bloggersfriend.isExecuted) {
							return;
						}
						if (bloggersfriendVars.isCustomerReviewEnabled) {
							if (typeof tb_pathToImage === 'undefined') {
								tb_pathToImage = bloggersfriendVars.thickboxUrl + '/loadingAnimation.gif';
							}
							if (typeof tb_closeImage === 'undefined') {
								tb_closeImage = bloggersfriendVars.thickboxUrl + '/tb-close.png';
							}
						}
						$.bloggersfriend.isFadeInEnabled = bloggersfriendVars.isFadeInEnabled;
						$.bloggersfriend.isCustomerReviewEnabled = bloggersfriendVars.isCustomerReviewEnabled;
						$.bloggersfriend.isTrackEventEnabled = bloggersfriendVars.isTrackEventEnabled;
						$.bloggersfriend.resource = bloggersfriendVars.resource;
						$.bloggersfriend.template(bloggersfriendVars.regionTemplate);
						$.bloggersfriend.render(bloggersfriendVars.items);
						$.bloggersfriend.isExecuted = true;
					}
					if (bloggersfriendVars.isFadeInEnabled) {
						setTimeout(function() {
							render();
						}, 1000);
					} else {
