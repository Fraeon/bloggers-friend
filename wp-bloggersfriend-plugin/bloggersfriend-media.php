...
/**
 * @var bloggersfriend $bloggersfriend
 */
global $wpdb, $wp_query, $wp_locale, $type, $tab, $post_mime_types, $bloggersfriend;
add_filter( 'media_upload_tabs', array( $bloggersfriend, 'media_upload_tabs' ) );
media_upload_header();
$text_domain       = $bloggersfriend->text_domain;
$access_key_id     = $bloggersfriend->settings['idAccessKey'];
$secret_access_key = $bloggersfriend->settings['idAccessSecret'];
$error             = (empty( $access_key_id ) || empty( $secret_access_key ));

bloggersfriend_aws ( $bloggersfriend );
?>

<div id="media_amazon">
	<?php if ( empty( $access_key_id ) || empty( $secret_access_key ) ) : ?>
		<div class="updated error">
			<p><?php printf( __( 'The AWS Access Key ID or AWS Secret Access Key is empty. Please specify it in <a href="%s" target="_blank">settings</a>.', $text_domain ), $bloggersfriend->option_page_url ); ?></p>
		</div>
	<?php endif ?>
	<form id="search_form" class="bloggersfriend_search_form" method="get" action="<?php bloginfo( 'wpurl' ); ?>/wp-admin/admin-ajax.php">
		<input type="hidden" name="tab" value="<?php echo esc_attr( $tab ); ?>"/>
		<input type="hidden" id="search_page" name="ItemPage" value="1"/>
		<?php if ( 'bloggersfriend_keyword' == $tab ) : ?>
			<fieldset>
				Store: <select id="search_locale" name="CountryCode"></select>
			</fieldset>
			<input type="text" id="search_query" name="Keywords" placeholder="<?php _e( 'Input URL', $text_domain ) ?>"/>
		<?php elseif ( 'bloggersfriend_id' == $tab ) : ?>
			<fieldset>
...
(function ($) {
	$(document).ready(function () {
		var $insert = $('#insert'),
			...
			loading = false,
			selectedItem,
			defaultLocale = '<?php echo esc_js( $bloggersfriend->default_country_code() ); ?>',
			countries = <?php echo json_encode( $bloggersfriend->countries );?>,
			searchIndexes = <?php echo json_encode( $bloggersfriend->search_indexes );?>
			;
		...
		$.bloggersfriend.initTemplate();
		$.template('bloggersfriendSearchIndexHeaderTpl', '<?php _e( '<h3 class="searchindex"><a href="#" rel="${IndexName}">${Label}</a> (${Results} hits)</h3>', $text_domain )?>');
		$.template('bloggersfriendSearchPagerTpl', '<?php _e( '<div class="searchpager">{{if prev}}<button class="button prev">Prev</button>{{/if}}${startIndex} - ${endIndex} / ${totalResults}{{if next}}<button class="button next">Next</button>{{/if}}</div>', $text_domain )?>');
		$.template('bloggersfriendSearchItemTpl',
			[
				'<li id="asin_${ASIN}" class="bloggersfriend_searchitem">',
				...
				'{{else}}',
				'<img src="<?php echo esc_attr( $bloggersfriend->url ); ?>/images/noimage-small.jpg" alt="${Title}"/>',
				'{{/if}}',
				'</a>',
				...
				'{{if EditorialReview}}&nbsp; ${EditorialReview}<br />{{/if}}',
				'<button id="btn_${ASIN}" class="button select"><?php _e( 'Select', $text_domain )?></button>',
				'<div class="bloggersfriend_footer"></div>',
				'</li>'
			].join(''));
		...
		function request(params) {
			params = params || {};
			params['action'] = 'bloggersfriend_search';
		...
		function onLoaded(data, params) {
			...
			if (!data || !data.items || data.items.length == 0) {
				if (data && data.success) {
					...
					var msg = (data) ? (data.message || 'Error') : 'bloggersfriend Search Error';
			...
			if (params.SearchIndex == 'Blended') {
				$.each(data.resultMap.SearchIndex, function (i, searchIndex) {
					...
					$results.append($.tmpl("bloggersfriendSearchIndexHeaderTpl", searchIndex));
					var $ul = $('<ul/>');
					$ul.append($.tmpl("bloggersfriendSearchItemTpl", subItems));
					$results.append($ul);
				});
			} else {
				var os = data.os;
				if (data.operation == 'ItemSearch') {
					...
					var $pager = $.tmpl("bloggersfriendSearchPagerTpl", os);
					$results.append($pager);
				}
				var $ul = $('<ul/>');
				$ul.append($.tmpl("bloggersfriendSearchItemTpl", items));
				$results.append($ul);
				if ($pager) {
		...
		var $container = $('#media_amazon'),
			...
			$templateSelect = $('#select_template input').change(function () {
				...
				if (html) {
					...
					var $item = $.bloggersfriend.tmpl(selectedItem, $.bloggersfriend.formatTmplName(selectedItem.Tmpl));
					$preview.empty().append($item);
		...
		// display template
		{
			$.template('linkAmazonItem', '<a href="${DetailPageURL}" title="${Title}" target="_blank">${Title}</a> - ${ListPrice.FormattedPrice}');
			$.template('smallImageAmazonItem',
				[
					'<a href="${DetailPageURL}" title="${Title}" target="_blank">',
					'{{if SmallImage}}',
					'<img src="${SmallImage.src}" width="${SmallImage.width}" height="${SmallImage.height}" alt="${Title}"/>',
					'<p>${ListPrice.FormattedPrice}</p>',					
					'{{/if}}',
					'</a>'
				].join(''));
			...
			$.template('shortCodeAmazonItem', '[bloggersfriend asin="${ASIN}" locale="${CountryCode}"{{if Tmpl}} tmpl="${Tmpl}"{{/if}} title="${EscapeTitle}"]');
		}
		...
		function showTemplateForm(item) {
			...
			$('#template_bloggersfriend').attr('checked', 'checked').change();
		}
